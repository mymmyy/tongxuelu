...
<div id="main">
  ...
  <div class="head_nav">
    ...
    <div class="w960">
      ...
      <a href="${pageContext.request.contextPath}/space/uploadhead/${sessionScope.spaceUser.userId}" class="uploadhead"><div class="head-avatar"><img src="${sessionScope.spaceUser.imgurl}" alt=""></div></a>
    </div>
  </div>
      ...
      <div class="m_main">
        <div class="inner">
          <div class="hd">
            ...
            <c:choose>
	         <c:when test="${sessionScope.existUser.userId == sessionScope.spaceUser.userId}"><!-- 如果加入自己的空间-->
             <a style="float:right;text-align:center;width:100px;height:40px;background-color:#C1C1C1;" role="button" data-category="UserAccount" data-action="login" data-toggle="modal" href="#login-modal">完善资料</a>
             </c:when>
	          </c:choose>
			</h3>
          </div>
...
<a style="opacity: 0;" id="topcontrol" href="javascript:;" title="返回顶部"></a>
 <div class="modal in" id="login-modal"> <a class="close" data-dismiss="modal">×</a>
        ...
        <form style="margin-top:55px;" class="login-form clearfix" method="post" action="#" onSubmit="return checkSubmit()">
          ...
          <input style="margin-top:-12px;height:40px;" id="submit" type="button" name="type" class="button-blue login" value="提&nbsp;交" onclick="checkSubmit()">
        </form>
</div>
</body>
 ...
 function checkSubmit(){
	 ...
	 if(flag==true){
		 var nickname=$(".nickname").val();
			...
			$.post("/tongxuelu/user/updateUserInfo",{"nickname":nickname,"gender":gender,"birthday":birth,"email":email,"address":address
				,"school":school,"profession":profession,"signal":signal},function(data){
					if(data>0)
						location.reload();	
				}) 
	 }
